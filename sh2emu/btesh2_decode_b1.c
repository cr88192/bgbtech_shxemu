	...
	btesh2_vaddr pc, u16 opw, u16 opw2)
{
	...
	case 0x2:
		...
		case 0x4:
			...
			op->Run=BTSH_Op_FMOVS_RegLdDisp;
			break;
		case 0x5:
			op->rn=(opw2>>8)&15;
			op->rm=(opw2>>4)&15;
...
int BTESH2_DecodeOpcode_BJX1_8C0(BTESH2_CpuState *cpu, BTESH2_Opcode *op,
	btesh2_vaddr pc, u16 opw, u16 opw2)
{
	int i, j, k;
...
//	switch(opw2>>12)
	...
	{
	case 0x0:
		BTESH2_DecodeOpcode_BJX1_8C0(cpu, op, pc, opw, opw2);
		break;
	case 0x3:
		op->ro=(opw2>>8)&15;
		op->rn=(opw2>>4)&15;
		op->rm=(opw2>>0)&15;
		if(opw&0x0004)
			op->ro+=BTESH2_REG_RHI;
		if(opw&0x0002)
			op->rn+=BTESH2_REG_RHI;
		if(opw&0x0001)
			op->rm+=BTESH2_REG_RHI;
