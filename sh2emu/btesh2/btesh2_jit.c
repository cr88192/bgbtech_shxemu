...
//			reg, UAX_REG_RCCTX, offsetof(BTESH2_CpuState, regs)+(idx*4));
		UAX_AsmInsnRegLdRegDisp(jctx, UAX_OP_MOVSD,
			reg, UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+(idx*8));
	}else
	{
//		UAX_AsmInsnRegLdRegDisp(jctx, UAX_OP_MOV,
//			reg, UAX_REG_RCCTX, offsetof(BTESH2_CpuState, regs)+(idx*4));
...
//			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, regs)+(idx*4), reg);
		UAX_AsmInsnStRegDispReg(jctx, UAX_OP_MOVSS,
			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+(idx*8), reg);
	}else
	{
		if(UAX_Asm_RegIsQWordP(reg))
		{
...
//				((BTESH2_REG_RHI+idx)*4), (s32)(val>>32));
		UAX_AsmInsnStRegDispImm32(jctx, UAX_OP_MOV,
			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+(idx*8+0),
			(s32)val);
		UAX_AsmInsnStRegDispImm32(jctx, UAX_OP_MOV,
			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+(idx*8+4),
			(s32)(val>>32));
	}else
	{
//		UAX_AsmInsnStRegDispImm32(jctx, UAX_OP_MOV,
//			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, regs)+(idx*4), val);
		UAX_AsmInsnStRegDispImm32(jctx, UAX_OP_MOV,
			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+(idx*8),
			(s32)val);
	}
	return(0);
}
...
//			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, regs)+(idx*4), val);
		UAX_AsmInsnStRegDispImm32(jctx, op,
			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+(idx*8), val);
//		UAX_AsmInsnStRegDispImm32(jctx, op,
//			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, regs)+(idx*8+4), val>>31);
	}
	return(0);
}
...
int BTESH2_JitMovVMRegVMReg(UAX_Context *jctx, int didx, int sidx)
{
	...
	if(jctx->isjq)
	{
		...
		UAX_AsmInsnRegLdRegDisp(jctx, UAX_OP_MOV,
			...
			offsetof(BTESH2_CpuState, qregs)+(sidx*8));
		UAX_AsmInsnStRegDispReg(jctx, UAX_OP_MOV,
			UAX_REG_RCCTX, offsetof(BTESH2_CpuState, qregs)+
				(didx*8), UAX_REG_RCX);
#endif
...
//		trj=BTESH2_JTraceForAddr(cpu, tr->jmppc);
		if(trj)
		{
			if(tr->jmppc != trj->srcpc)
				__debugbreak();
		
			BTESH2_JitLinkTrace(cpu, trj);
			tr->trjmpnext=trj;
			BTESH2_ValidateTrace(cpu, trj);
		}
	}
#endif
...
//		UAX_AsmInsnRegImm(jctx, UAX_OP_MOV, UAX_REG_RAX, (s64)(tr->trjmpnext));
//		UAX_AsmInsnRegReg(jctx, UAX_OP_XOR, UAX_REG_RAX, UAX_REG_RAX);
