...
//		BTESH2_JitXorVMReg(jctx, op->rn, UAX_REG_EAX);
	...
	if((op->nmid==BTESH2_NMID_DMULS) && (op->fmid==BTESH2_FMID_REGREG))
	{
		BTESH2_JitLoadVMReg(jctx, op->rm, UAX_REG_EAX);
		BTESH2_JitLoadVMReg(jctx, op->rn, UAX_REG_ECX);
		UAX_AsmInsnReg(jctx, UAX_OP_IMUL, UAX_REG_ECX);
		BTESH2_JitStoreVMReg(jctx, BTESH2_REG_MACL, UAX_REG_EAX);
		BTESH2_JitStoreVMReg(jctx, BTESH2_REG_MACH, UAX_REG_EDX);
		return(1);
	}

	if((op->nmid==BTESH2_NMID_DMULU) && (op->fmid==BTESH2_FMID_REGREG))
	{
		BTESH2_JitLoadVMReg(jctx, op->rm, UAX_REG_EAX);
		BTESH2_JitLoadVMReg(jctx, op->rn, UAX_REG_ECX);
...
//		UAX_AsmInsnRegImm(jctx, UAX_OP_SAR, UAX_REG_EDX, 31);
		...
		UAX_EmitLabel(jctx, l2);
		BTESH2_JitStoreVMReg(jctx, op->ro, UAX_REG_EDX);
		return(1);
	}
