...
#if 1
	case 0x00: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_PTEH); break;
	case 0x01: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_PTEL); break;
	case 0x02: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_TTB); break;
	case 0x03: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_TEA); break;
	case 0x04: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_MMUCR); break;

	case 0x08: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_TRAEVT); break;
	case 0x09: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_EXPEVT); break;
	case 0x0A: v=BTESH2_GetRegDWord(cpu, BTESH2_REG_INTEVT); break;
#endif

	default:
		printf("MMREG: Unhandled Get A=%08X\n", (int)reladdr);
//		cpu->regs[BTESH2_REG_FLA]=sp->base+reladdr;
		BTESH2_SetRegQWord(cpu, BTESH2_REG_TEA, sp->base+reladdr);
...
#if 1
	case 0x00: BTESH2_SetRegDWordL(cpu, BTESH2_REG_PTEH, val); break;
	case 0x01: BTESH2_SetRegDWordL(cpu, BTESH2_REG_PTEL, val); break;
	case 0x02: BTESH2_SetRegDWordL(cpu, BTESH2_REG_TTB, val); break;
	case 0x03: BTESH2_SetRegDWordL(cpu, BTESH2_REG_TEA, val); break;
	case 0x04:
		BTESH2_SetRegDWordL(cpu, BTESH2_REG_MMUCR, val);
		BTESH2_SetupUpdateFMMU(cpu);
		break;

	case 0x08: BTESH2_SetRegDWordL(cpu, BTESH2_REG_TRAEVT, val); break;
	case 0x09: BTESH2_SetRegDWordL(cpu, BTESH2_REG_EXPEVT, val); break;
	case 0x0A: BTESH2_SetRegDWordL(cpu, BTESH2_REG_INTEVT, val); break;
#endif
...
//		cpu->regs[BTESH2_REG_SP]=0x0000FFFC;
		...
		cpu->qregs[BTESH2_REG_SR].q&=0x800013F3ULL;
		cpu->qregs[BTESH2_REG_SR].q|=0x000003F3ULL;
		cpu->qregs[BTESH2_REG_SP].q=0x8C00FFFC;
	}else
	{
		img=BTESH2_AllocMemoryImage(0);
	//	BTESH2_MemoryDefineSpan(img, 0x00000000, 0x00003FFF, NULL, "SRAM");
...
//			mapname=NULL;
	...
	//	cpu->regs[BTESH2_REG_PC]=0x00003FFC;
		...
		cpu->qregs[BTESH2_REG_SR].q&=0x800013F3ULL;
		cpu->qregs[BTESH2_REG_SR].q|=0x000003F3ULL;
		cpu->qregs[BTESH2_REG_SP].q=0x0000FFFC;
...
//	l=99999999;
	...
	printf("Boot PC=%08X\n", BTESH2_GetRegDWord(cpu, BTESH2_REG_SR));
//	t0=clock();
