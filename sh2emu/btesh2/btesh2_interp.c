...
//	printf("@=%08X N=%08X\n", tr->srcpc, tr->nxtpc);
	for(i=0; i<tr->nops; i++)
	{
		...
		if(op->pc&1)
		{
			...
			{
				printf("  %08X_%08X",
					(u32)(op->pc>>32), (u32)op->pc);
			}else
			{
				printf("  %08X", (u32)op->pc);
			}
			
			k=((op->opw&15)<<16)|op->opw2;
			switch(op->pc&7)
			{
			case 1:
				printf("xxxx_xxxx_xxx%01X_%04X  ",
					(k>>16)&15, k&65535);
				break;
			case 3:
				printf("xxxx_xx%02X_%3Xx_xxxx  ",
					(k>>12)&255, k&4095);
				break;
			case 5:
				printf("x%03X_%02Xxx_xxxx_xxxx  ",
					(k>>8)&4095, k&255);
				break;
			}
			
			laspc=op->pc+2;
		}else if(op->fl&BTESH2_OPFL_EXTRAWORD)
		{
...
#if 0
	cpu->dbgpad1=0x12345678;
	cpu->dbgpad2=0x12345678;
	cpu->dbgpad3=0x12345678;
	cpu->dbgpad4=0x12345678;
	cpu->dbgpad5=0x12345678;
	cpu->dbgpad6=0x12345678;
	cpu->dbgpad7=0x12345678;
	cpu->dbgpad8=0x12345678;
#endif
...
//	while((l--)>0)
	while((cpu->tr_runlim--)>0)
	{
		...
		{
			pc=BTESH2_GetRegQWord(cpu, BTESH2_REG_PC);
			if(pc!=lpc)
			{
				if(cpu->logpc)
				{
					if(cpu->nlogpc<cpu->mlogpc)
					{
						cpu->logsp[cpu->nlogpc]=
							BTESH2_GetRegQWord(cpu, BTESH2_REG_SP);
...
//					BTESH2_GetRegDWordH(cpu, BTESH2_REG_PC];
					pc>>32;
...
//		printf("PC=%08X\n", BTESH2_GetRegQWord(cpu, BTESH2_REG_PC));

		if(!trnxt)
		{
			tr=BTESH2_TraceForAddr(cpu, BTESH2_GetRegQWord(cpu, BTESH2_REG_PC));
		}else
		{
			tr=trnxt;
		}
...
#if 0
	if(cpu->dbgpad1!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad2!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad3!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad4!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad5!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad6!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad7!=0x12345678)
		{ __debugbreak(); }
	if(cpu->dbgpad8!=0x12345678)
		{ __debugbreak(); }
#endif
...
int BTESH2_DumpRegs_Reg32(BTESH2_CpuState *cpu)
{
	...
	printf("R0   =%08X | R1  =%08X | R2   =%08X | R3   =%08X\n",
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R0),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R1),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R2),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R3));
	printf("R4   =%08X | R5  =%08X | R6   =%08X | R7   =%08X\n",
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R4),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R5),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R6),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R7));
	printf("R8   =%08X | R9  =%08X | R10  =%08X | R11  =%08X\n",
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R8),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R9),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R10),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R11));
	printf("R12  =%08X | R13 =%08X | R14  =%08X | R15  =%08X\n",
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R12),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R13),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R14),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_R15));

	if(cpu->arch==BTESH2_ARCH_SH4)
	{
		printf("Banked GPRs:\n");
		printf("R0_B =%08X | R1_B=%08X | R2_B =%08X | R3_B =%08X\n",
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+0),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+1),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+2),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+3));
		printf("R4_B =%08X | R5_B=%08X | R6_B =%08X | R7_B =%08X\n",
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+4),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+5),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+6),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_RBANK+7));
	}

	printf("Control/Status:\n");
	printf("SR   =%08X | GBR =%08X | VBR  =%08X | fla  =%08X\n",
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_SR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_GBR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_VBR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_FLA));
	printf("MACH =%08X | MACL=%08X | PR   =%08X | PC   =%08X\n",
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_MACH),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_MACL),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_PR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_PC));

	if(cpu->arch==BTESH2_ARCH_SH4)
	{
		printf("FPSCR=%08X | FPUL=%08X | flb  =%08X | MMUCR=%08X\n",
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_FPSCR),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_FPUL),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_FLB),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_MMUCR));
		printf("PTEH =%08X | PTEL=%08X | TTB  =%08X | TEA  =%08X\n",
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_PTEH),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_PTEL),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_TTB),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_TEA));

		printf("SSR  =%08X | SPC =%08X | SGR  =%08X | DBR  =%08X\n",
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_SSR),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_SPC),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_SGR),
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_DBR));
	}
...
//	for(i=0; i<8; i++)
	for(i=0; i<16; i++)
	{
		...
		printf("R%c%c  =%08X_%08X | R%c%c  =%08X_%08X\n",
			c0, c1,
			BTESH2_GetRegDWordH(cpu, j+0),
			BTESH2_GetRegDWordL(cpu, j+0),
			c2, c3,
			BTESH2_GetRegDWordH(cpu, j+1),
			BTESH2_GetRegDWordL(cpu, j+1));
	}
...
//	if(cpu->arch==BTESH2_ARCH_SH4)
	...
	printf("SR   =%08X_%08X | GBR  =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_SR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_SR),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_GBR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_GBR));
	printf("VBR  =%08X_%08X | fla  =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_VBR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_VBR),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_FLA),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_FLA));
	printf("MACH =%08X_%08X | MACL =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_MACH),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_MACH),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_MACL),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_MACL));
	printf("PR   =%08X_%08X | PC   =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_PR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_PR),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_PC),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_PC));
	printf("FPSCR=%08X_%08X | FPUL =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_FPSCR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_FPSCR),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_FPUL),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_FPUL));
	printf("flb  =%08X_%08X | MMUCR=%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_FLB),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_FLB),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_MMUCR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_MMUCR));
	printf("PTEH =%08X_%08X | PTEL =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_PTEH),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_PTEH),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_PTEL),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_PTEL));
	printf("TTB  =%08X_%08X | TEA  =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_TTB),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_TTB),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_TEA),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_TEA));
	printf("SSR  =%08X_%08X | SPC  =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_SSR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_SSR),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_SPC),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_SPC));
	printf("SGR  =%08X_%08X | DBR  =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_SGR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_SGR),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_DBR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_DBR));
	printf("TRAEV=%08X_%08X | EXPEV=%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_TRAEVT),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_TRAEVT),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_EXPEVT),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_EXPEVT));
	printf("INTEV=%08X_%08X | ZZR  =%08X_%08X\n",
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_INTEVT),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_INTEVT),
		BTESH2_GetRegDWordH(cpu, BTESH2_REG_ZZR),
		BTESH2_GetRegDWordL(cpu, BTESH2_REG_ZZR));
...
int BTESH2_DumpRegs(BTESH2_CpuState *cpu)
{
	...
	if((BTESH2_GetRegDWordH(cpu, BTESH2_REG_SR)&BTESH2_SRFL_JQ) ||
		(BTESH2_GetRegDWordL(cpu, BTESH2_REG_SR)&BTESH2_SRFL_JQ))
//	if(1)
	{
...
//	BTESH2_DumpRegs_Reg64(cpu);
	...
	{
		printf("%08X % 4d: ",
			BTESH2_GetRegDWordL(cpu, BTESH2_REG_SP)+i*4*4, i*4*4);
		for(j=0; j<4; j++)
		{
			if(j)printf(" ");
			k=i*4+j;
			l=BTESH2_GetAddrDWord(cpu,
				BTESH2_GetRegDWordL(cpu, BTESH2_REG_SP)+k*4);
			printf("%08X", l);
		}
		printf("\n");
	}
