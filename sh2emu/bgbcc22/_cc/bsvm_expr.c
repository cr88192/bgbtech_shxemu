...
void BGBCC_BSVM_CompileAssign(BGBCC_State *ctx, BCCX_Node *l)
{
	BCCX_Node *c, *ct, *cv, *t;
	char *s;
	int i, j;
	...
	if(BGBCC_IsUnaryP(l, "*"))
	{
		BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(l, "value"));
	...
	{
		BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(l, "array"));
		BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(l, "index"));
...
void BGBCC_BSVM_CompileExprList(BGBCC_State *ctx, BCCX_Node *lst)
{
	BCCX_Node *cur;
	...
	while(cur)
	{
		BGBCC_BSVM_CompileExpr(ctx, cur);
		cur=BCCX_Next(cur);
	}
}

void BGBCC_BSVM_CompileExprListReverse(BGBCC_State *ctx, BCCX_Node *lst)
{
	BCCX_Node *stk[128];
	BCCX_Node *cur;
	int i;
	...
	while(i--)BGBCC_BSVM_CompileExpr(ctx, stk[i]);
}

void BGBCC_BSVM_CompileArgsList(BGBCC_State *ctx, BCCX_Node *lst)
{
//	BCCX_Node *stk[128];
//	BCCX_Node *cur;
//	int i;
...
//	i=0; cur=lst;
//	while(cur) { stk[i++]=cur; cur=BCCX_Next(cur); }
//	while(i--) { BGBCC_BSVM_CompileExpr(ctx, stk[i]); bgbcc_print(ctx, ", "); }

	BGBCC_BSVM_CompileExprList(ctx, lst);
}

void BGBCC_BSVM_CompileFuncall(BGBCC_State *ctx, BCCX_Node *l)
{
	char *s0, *s1, *s2;
	BCCX_Node *c, *d, *t, *u, *v;
	int i, j;
	...
	if(s0)
	{
		...
		if(i)
		{
			...
			c=BCCX_Fetch(l, "args");
//			BGBCC_BSVM_CompileExprListReverse(ctx,
//				BCCX_Fetch(l, "args"));
			BGBCC_BSVM_CompileExprList(ctx,
				BCCX_Fetch(l, "args"));
...
//		bgbcc_print(ctx, "%s( ", s0);
		bgbcc_print(ctx, "mark; ");
		BGBCC_BSVM_CompileArgsList(ctx, BCCX_Fetch(l, "args"));
...
//		bgbcc_print(ctx, ") ");
	...
	BGBCC_BSVM_CompileArgsList(ctx,
		BCCX_Fetch(l, "args"));
	BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(l, "func"));
	bgbcc_print(ctx, "call; ");
	return;
}

void BGBCC_BSVM_CompileForm(BGBCC_State *ctx, BCCX_Node *l)
{
	byte *ips[64];
	char *s0, *s1, *s2;
	...
	if(BCCX_TagIsP(l, "sizeof"))
	{
		...
		if(!t)
		{
			printf("BGBCC_BSVM_CompileForm: sizeof: bad AST\n");
			bgbcc_print(ctx, "0 ");
			return;
		}
	...
	if(BCCX_TagIsP(l, "if"))
	{
		...
		if(i==1)
		{
			BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(l, "then"));
			return;
		}
		if(i==0)
		{
			t=BCCX_Fetch(l, "else");
			if(t)BGBCC_BSVM_CompileExpr(ctx, t);
			return;
		}
...
//		bgbcc_print(ctx, "beginu ");
		...
		BGBCC_BSVM_CompileExprT(ctx, BCCX_Fetch(l, "then"));
//		bgbcc_print(ctx, "setu ");
		...
		t=BCCX_Fetch(l, "else");
		if(t)BGBCC_BSVM_CompileExprT(ctx, t);
...
//		bgbcc_print(ctx, "endu ");
	...
	if(BCCX_TagIsP(l, "unary"))
	{
		...
		if(BCCX_AttrIsP(l, "op", "&"))
		{
			...
			{
				BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(t, "value"));
			...
			{
				BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(t, "array"));
				BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(t, "index"));
	...
	{
		BGBCC_BSVM_CompileFuncall(ctx, l);
		return;
	}
	...
	if(BCCX_TagIsP(l, "methodcall"))
	{
		bgbcc_print(ctx, "mark; ");
		c=BCCX_Fetch(l, "args");
//		BGBCC_BSVM_CompileExprListReverse(ctx, c);
		BGBCC_BSVM_CompileExprList(ctx, c);
		BGBCC_BSVM_CompileExpr(ctx, BCCX_Fetch(l, "value"));
...
void BGBCC_BSVM_CompileExprT(BGBCC_State *ctx, BCCX_Node *l)
{
	long long li;
	float f;
	BCCX_Node *t, *c, *ct, *cv;
...
//				{ bgbcc_print(ctx, "push_undef; "); return; }
	...
	BGBCC_BSVM_CompileForm(ctx, l);
	return;
}

void BGBCC_BSVM_CompileExpr(BGBCC_State *ctx, BCCX_Node *l)
{
	int i;

	i=ctx->tail;
	ctx->tail=0;
	BGBCC_BSVM_CompileExprT(ctx, l);
	ctx->tail=i;
}

void BGBCC_BSVM_CompileExprTail(BGBCC_State *ctx, BCCX_Node *l)
{
	int i;
