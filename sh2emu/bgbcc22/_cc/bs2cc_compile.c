...
int BGBCC_BS2IL_CompileSwitch(BGBCC_State *ctx, BCCX_Node *l)
{
	...
	BGBCC_BS2IL_CompileExpr(ctx, BCCX_Fetch(l, "cond"));
	...
	while(c)
	{
		...
		{
			bgbcc_print(ctx, "dup ");
			BGBCC_BS2IL_CompileExpr(ctx, BCCX_Fetch(c, "value"));
			BGBCC_BS2IL_CompileJmpCond(ctx, "==", cl[i++]);
			c=BCCX_Next(c);
			continue;
		}
		...
		{
			BGBCC_BS2IL_CompileJmp(ctx, cl[i++]);
			c=BCCX_Next(c);
			continue;
		}
		c=BCCX_Next(c);
	...
	while(c)
	{
		if(BCCX_TagIsP(c, "case") ||
			BCCX_TagIsP(c, "case_default"))
		{
			BGBCC_BS2IL_EmitLabel(ctx, cl[i++]);
			c=BCCX_Next(c);
			continue;
		}
...
//	bgbcc_print(ctx, "\n");
	...
	{
		BGBCC_BS2IL_CompileSwitch(ctx, l);
		return;
	}

	if(BCCX_TagIsP(l, "goto"))
	{
		BGBCC_BS2IL_CompileJmp(ctx, BCCX_Get(l, "name"));
		return;
	}

	if(BCCX_TagIsP(l, "label"))
	{
		BGBCC_BS2IL_EmitLabel(ctx, BCCX_Get(l, "name"));
		return;
	}
	...
	if(BCCX_TagIsP(l, "if"))
	{
		...
		if(i==1)
		{
			BGBCC_BS2IL_CompileStatement(ctx, BCCX_Fetch(l, "then"));
			return;
		}
		if(i==0)
		{
			t=BCCX_Fetch(l, "else");
			if(t)BGBCC_BS2IL_CompileStatement(ctx, t);
			return;
		}
		...
		if(t)
		{
			...
			BGBCC_BS2IL_CompileStatement(ctx, t);
			s0=s1;
		}
		BGBCC_BS2IL_EmitLabel(ctx, s0);
		return;
	...
	if(BCCX_TagIsP(l, "for"))
	{
		...
		BGBCC_BS2IL_CompileStatement(ctx, BCCX_Fetch(l, "init"));

//		BGBCC_BS2IL_EmitLabel(ctx, s1);
		BGBCC_BS2IL_EmitLabel(ctx, s0);
		BGBCC_BS2IL_CompileJCF(ctx, BCCX_Fetch(l, "cond"), s2);

		BGBCC_BS2IL_CompileStatement(ctx, BCCX_Fetch(l, "body"));

		BGBCC_BS2IL_EmitLabel(ctx, s1);
		BGBCC_BS2IL_CompileStatement(ctx, BCCX_Fetch(l, "step"));
//		BGBCC_BS2IL_CompileContinue(ctx);
		BGBCC_BS2IL_CompileJmp(ctx, s0);

		BGBCC_BS2IL_EmitLabel(ctx, s2);
	...
	if(BCCX_TagIsP(l, "while"))
	{
		...
		BGBCC_BS2IL_CompileJCF(ctx, t, s2);
		BGBCC_BS2IL_CompileStatement(ctx, BCCX_Fetch(l, "body"));
		BGBCC_BS2IL_CompileContinue(ctx);
		BGBCC_BS2IL_EmitLabel(ctx, s2);
	...
	if(BCCX_TagIsP(l, "do_while"))
	{
		...
		BGBCC_BS2IL_CompileJCT(ctx, t, s1);
		BGBCC_BS2IL_EmitLabel(ctx, s2);
...
void BGBCC_BS2IL_EmitVar(BGBCC_State *ctx, char *name, BCCX_Node *ty, BCCX_Node *v)
{
	...
	if(v)
	{
		...
		BGBCC_BS2IL_EmitVarValueR(ctx, v);
	}
...
void BGBCC_BS2IL_EmitVar2(BGBCC_State *ctx, char *name, BCCX_Node *ty, BCCX_Node *v)
{
	...
	BGBCC_BS2IL_EmitVar(ctx, name, ty, v);
...
void BGBCC_BS2IL_EmitVarProperty(BGBCC_State *ctx, BCCX_Node *l)
{
	...
	if(v)
	{
		...
		BGBCC_BS2IL_EmitVar3(ctx, s2, s1);
	}
...
//	BGBCC_BS2IL_EmitVar3(ctx, "new", s1);
//	BGBCC_BS2IL_CompileBlock(ctx, t, s, n, v);
...
void BGBCC_BS2IL_CompileVarProperty(BGBCC_State *ctx, BCCX_Node *l)
{
	...
	if(v)
	{
		...
		v=BCCX_Fetch(v, "body");
		BGBCC_BS2IL_CompileBlock(ctx, t, s2, NULL, v);
...
//		BGBCC_BS2IL_EmitVar3(ctx, s2, s1);
	...
	if(v)
	{
		...
		v=BCCX_Fetch(v, "body");
		BGBCC_BS2IL_CompileBlock(ctx, NULL, s2, n, v);
...
void BGBCC_BS2IL_CompileVarStatement(BGBCC_State *ctx, BCCX_Node *l)
{
	...
	if(BCCX_TagIsP(l, "vars"))
	{
		...
		while(c)
		{
			...
			BGBCC_BS2IL_EmitVar2(ctx, s, t, v);
...
//		BGBCC_BS2IL_CompileBlock(ctx, t, s, n, v);
	...
	{
		BGBCC_BS2IL_EmitVarProperty(ctx, l);
		return;
	}

	if(BGBCC_BS2IL_CompileVarDummyP(ctx, l))
		return;
...
//			BGBCC_BS2IL_EmitVar2(ctx, s, t, v);
			BGBCC_BS2IL_EmitTopVar(ctx, s, t, v);
	...
	if(BCCX_TagIsP(l, "property"))
	{
		BGBCC_BS2IL_CompileVarProperty(ctx, l);
		return;
	}
...
BCCX_Node *BGBCC_BS2IL_CompileBlock(BGBCC_State *ctx,
...
//	BGBCC_BS2IL_EmitSigProto(ctx, type, name, args);
	...
	BGBCC_BS2IL_EmitVar4(ctx, ctx->cf_n, ctx->cf_ty);
	...
	while(c)
	{
		...
		bgbcc_print(ctx, "\t");
		t=BCCX_FindTag(c, "type");
		BGBCC_BS2IL_EmitVar(ctx, BCCX_Get(c, "name"), t, NULL);
		c=BCCX_Next(c);
		bgbcc_print(ctx, "\n");
	}
	...
	{
		bgbcc_print(ctx, "\t");
		BGBCC_BS2IL_CompileVarStatement(ctx, c);
		c=BCCX_Next(c);
		bgbcc_print(ctx, "\n");
	}
...
void BGBCC_BS2IL_EmitSigProto(BGBCC_State *ctx,
	BCCX_Node *type, char *name, BCCX_Node *args)
		{ BGBCC_EmitSigProto(ctx, type, name, args); }

BCCX_Node *BGBCC_BS2IL_CompileProto(BGBCC_State *ctx,
	BCCX_Node *type, char *name, BCCX_Node *args)
{
	BCCX_Node *c, *t, *n, *u;
	int i, j, k;
...
//		BGBCC_BS2IL_CompileVarStatement(ctx, c);
	...
	while(c)
	{
		BGBCC_BS2IL_CompileVarStatement2(ctx, c);
		c=BCCX_Next(c);
	}
#endif
...
BCCX_Node *BGBCC_BS2IL_CompileSProto(BGBCC_State *ctx, BCCX_Node *l)
{
	...
	t=BCCX_FindTag(l, "type");
	BGBCC_BS2IL_EmitVar4(ctx, BCCX_Get(l, "name"), t);
	bgbcc_print(ctx, "\n");
...
void BGBCC_BS2IL_CompileStructs(BGBCC_State *ctx)
{
	...
	while(c)
	{
		BGBCC_BS2IL_CompileTypedef(ctx, c);
		c=BCCX_Next(c);
	}
	...
	{
		l=c;
		if(BCCX_TagIsP(c, "struct"))BGBCC_BS2IL_CompileStruct(ctx, c);
		if(BCCX_TagIsP(c, "union"))BGBCC_BS2IL_CompileUnion(ctx, c);
//		if(BCCX_TagIsP(c, "enum"))BGBCC_BS2IL_CompileEnum(ctx, c);
		if(BCCX_TagIsP(c, "s_proto"))BGBCC_BS2IL_CompileSProto(ctx, c);
		if(BCCX_TagIsP(c, "func"))BGBCC_BS2IL_CompileSProto(ctx, c);
...
void BGBCC_BS2IL_EmitVarValue(BGBCC_State *ctx, BCCX_Node *v)
{
	...
	BGBCC_Error("BGBCC_BS2IL_EmitVarValue: Bad Value %s\n", BCCX_Tag(v));
}
...
void BGBCC_BS2IL_EmitVarValueR(BGBCC_State *ctx, BCCX_Node *v)
{
	...
	if(BCCX_TagIsP(v, "list"))
	{
		...
		while(c)
		{
			BGBCC_BS2IL_EmitVarValueR(ctx, c);
			bgbcc_print(ctx, " ");
			c=BCCX_Next(c);
		}
	...
	BGBCC_BS2IL_EmitVarValue(ctx, v);
}
...
void BGBCC_BS2IL_EmitVarValueR2(BGBCC_State *ctx, BCCX_Node *v)
{
	...
	if(BCCX_TagIsP(v, "list"))
	{
		...
		while(c)
		{
			BGBCC_BS2IL_EmitVarValueR2(ctx, c);
			bgbcc_print(ctx, " ");
			c=BCCX_Next(c);
		}
...
//		s1=name;
	...
	if(!ctx->cur_struct)
	{
		...
		if(i)
		{
			BGBCC_BS2IL_EmitTopArray(ctx, name, ty, v);
			return;
		}
	}
...
//	if(s2 && (*s2))
	...
	{
		bgbcc_print(ctx, "values { ");
		BGBCC_BS2IL_EmitVarValueR2(ctx, v);
		bgbcc_print(ctx, "} ");
	}
...
void BGBCC_BS2IL_CompileTopStatement(BGBCC_State *ctx, BCCX_Node *l)
{
	...
	if(BCCX_TagIsP(l, "vars"))
	{
		...
		while(c)
		{
			if(BCCX_TagIsP(c, "proto"))
			{
				...
				BGBCC_BS2IL_CompileProto(ctx, t, s, n);
			}
	...
	{
		BGBCC_BS2IL_CompileStruct(ctx, l);
		return;
	}

	if(BCCX_TagIsP(l, "union"))
	{
		BGBCC_BS2IL_CompileUnion(ctx, l);
		return;
	}

	if(BCCX_TagIsP(l, "class"))
	{
		BGBCC_BS2IL_CompileClass(ctx, l);
		return;
	}
...
//		c=BCCX_Fetch(l, "body");
		...
		if(n)
		{
			...
			while(c)
			{
				BGBCC_BS2IL_CompileTopStatement(ctx, c);
				c=BCCX_Next(c);
			}
			ctx->cur_ns=s1;
		}
...
char *BGBCC_BS2IL_CompileModule(char *name, BCCX_Node *l)
{
	...
	BGBCC_BS2IL_CompileStructs(ctx);
